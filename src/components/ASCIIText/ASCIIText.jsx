...
void main(){
  vUv=uv;float time=uTime*5.;float wf=uEnableWaves;
  vec3 t=position;
  t.x+=sin(time+position.y)*.5*wf;
  t.y+=cos(time+position.z)*.15*wf;
...
Math.map = function (n, s1, s2, d1, d2) {
  return ((n - s1) / (s2 - s1)) * (d2 - d1) + d1;
};
...
class AsciiFilter {
  ...
  setSize(w, h) {
    ...
    this.renderer.setSize(w, h); this.reset();
  ...
  asciify(ctx, w, h) {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        let gray = (0.3 * d[i] + 0.6 * d[i + 1] + 0.1 * d[i + 2]) / 255;
...
class CanvAscii {
  ...
  setMesh() {
    ...
    const bH = this.planeBaseHeight;
    this.geometry = new THREE.PlaneGeometry(bH * aspect, bH, 36, 36);
    this.material = new THREE.ShaderMaterial({
      vertexShader, fragmentShader, transparent: true,
      uniforms: {
        uTime: { value: 0 }, mouse: { value: 1 },
        uTexture: { value: this.texture }, uEnableWaves: { value: this.enableWaves ? 1 : 0 }
      }
    });
      ...
      if (width === 0 || height === 0) {
        observer = new IntersectionObserver(async ([entry]) => {
          ...
          if (entry.isIntersecting && entry.boundingClientRect.width > 0) {
            ...
            if (!cancelled) { asciiRef.current = await create(containerRef.current, w, h); asciiRef.current?.load(); }
          }
        }, { threshold: 0.1 });
  ...
  return (
    <div ref={containerRef} style={{ position: 'absolute', width: '100%', height: '100%' }}>
      <style>{`
        ...
        [data-ascii] canvas{position:absolute;left:0;top:0;width:100%;height:100%;image-rendering:pixelated}
        [data-ascii] pre{margin:0;user-select:none;padding:0;line-height:1em;text-align:left;position:absolute;left:0;top:0;
        background-image:radial-gradient(circle,#8B5CF6 0%,#06B6D4 50%,#8B5CF6 100%);
        background-attachment:fixed;-webkit-text-fill-color:transparent;-webkit-background-clip:text;z-index:9;mix-blend-mode:difference}
      `}</style>
    </div>
  );
}